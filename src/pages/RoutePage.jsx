...
// ONLY FOR uiSlice + orderSlice - ADDED
...
export default function RoutePage({
  ...
  // FROM uiSlice - REPLACES props: theme, page, routeParams, onNavigate
  const { page, theme, routeParams } = useSelector((s) => s.ui);
  // FROM orderSlice - REPLACES props: orders, onSubmitOrder
  const { orders } = useSelector((s) => s.orders);

  // REDUX FUNCTIONS - REPLACES PROPS
  const onNavigate = (payload) => dispatch(setPage(payload));
  const onSubmitOrder = (payload) => dispatch(submitOrder(payload)).unwrap();
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      setCheckoutError(err?.message || "Order could not be submitted.");
    }
